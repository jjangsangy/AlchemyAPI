...
class TestAPI(unittest.TestCase):
    ...
    @unittest.skip('Skipping imagetagging')
    def test_imagetagging(self):
        print('Checking imagetagging . . . ')
        response = self._api.interface('imagetagging', 'text', test_text)
        self.assertTrue(response['status'] == 'ERROR')
        response = self._api.interface('imagetagging', 'html', test_html)
        self.assertTrue(response['status'] == 'ERROR')
        response = self._api.interface('imagetagging', 'url', test_url)
        self.assertTrue(response['status'] == 'OK')
        response = self._api.interface('imagetagging', 'image', test_jpg, image=test_jpg, imagePostMode='raw')
        self.assertTrue(response['status'] == 'OK')
